...
fn main() {
    ...
    let (handshaker_send, handshaker_recv) = HandshakerBuilder::new()
        ...
        // manager back pressure (handshaker -> peer manager will
                ...
                .and_then(|(opt_item, disk_manager_recv)| {
                    ...
                    let result_future: Box<dyn Future<Item = Loop<(), _>, Error = ()>> = match opt_message {
                        ...
                        Some(Either::B(peer_message)) => {
                            Box::new(peer_manager_send.send(peer_message).map(|peer_manager_send| {
                                Loop::Continue((disk_manager_recv, disk_request_map, select_send, peer_manager_send))
                            }))
                        }
                        None => Box::new(future::ok(Loop::Continue((
    ...
    let (select_recv, piece_requests, cur_pieces) = core
        .run(future::loop_fn(
            (select_recv, piece_requests, 0),
    ...
    */
    ...
    core.run(
        handshaker_send
            .send(InitiateMessage::new(Protocol::BitTorrent, info_hash, peer_addr))
            .map_err(|_| ()),
    )
    .unwrap();
    ...
    let result: Result<(), ()> = core.run(future::loop_fn(
        (
            select_recv,
            map_peer_manager_send,
            piece_requests,
