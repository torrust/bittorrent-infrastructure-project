        ...
        W: Write,
    {
        match self {
            &PeerExtensionProtocolMessage::UtMetadata(ref msg) => {
                ...
                try!(message::write_length_id_pair(
                    &mut writer,
                    total_len,
    ...
    P: PeerProtocol,
{
    ...
    alt!(
        ...
        ignore_input!(switch!(header_bytes.as_ref(), throwaway_input!(tuple!(be_u32, be_u8, be_u8)),
            ...
            .map(|cust_ext| PeerExtensionProtocolMessage::Custom(cust_ext)))
    )
}
    ...
    P: PeerProtocol,
{
    ...
    let result = if lt_metadata_id == Some(id) {
        UtMetadataMessage::parse_bytes(bytes).map(|lt_metadata_msg| PeerExtensionProtocolMessage::UtMetadata(lt_metadata_msg))
    } else {
        Err(io::Error::new(
            io::ErrorKind::Other,
