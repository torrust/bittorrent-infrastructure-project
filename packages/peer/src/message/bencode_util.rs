...
where
    V: BRefAccess,
{
    CONVERT.lookup_and_convert_bytes(root, OUR_IP_KEY).ok().and_then(|ip_bytes| {
        if ip_bytes.len() == 4 {
...
{
    CONVERT
        .lookup_and_convert_bytes(root, CLIENT_IPV6_ADDR_KEY)
        .ok()
        .and_then(|ipv6_bytes| {
...
{
    CONVERT
        .lookup_and_convert_bytes(root, CLIENT_IPV4_ADDR_KEY)
        .ok()
        .and_then(|ipv4_bytes| {
            if ipv4_bytes.len() == 4 {
