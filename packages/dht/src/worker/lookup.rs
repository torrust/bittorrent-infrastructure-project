...
impl TableLookup {
        ...
        H: HandshakerTrait,
    {
        ...
        for (node, dist_to_beat) in nodes {
            ...
            self.active_lookups.insert(trans_id, (dist_to_beat, timeout));
...
fn insert_sorted_node(nodes: &mut Vec<(Distance, Node, bool)>, target: InfoHash, node: Node, pinged: bool) {
    let node_id = node.id();
